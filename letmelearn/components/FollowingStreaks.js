    ...
    <v-card>
      ...
      <v-card-text>
        ...
        <v-list v-else-if="streaks.length > 0" two-line>
          <template v-for="(item, index) in streaks">
            <v-list-tile :key="item.user.email" avatar>
              <v-list-tile-avatar>
                <img :src="item.user.picture || '/app/static/images/default-avatar.png'">
              </v-list-tile-avatar>

              <v-list-tile-content>
                <v-list-tile-title>
                  {{ item.user.name || item.user.email }}
                  <v-chip v-if="item.streak > 0" small :color="streakColor(item.streak)" text-color="white">
                ...
                <v-list-tile-sub-title>
                  ...
                  {{ item.today_minutes }} min today
                </v-list-tile-sub-title>
              </v-list-tile-content>
            </v-list-tile>
            <v-divider v-if="index < streaks.length - 1" :key="'divider-' + item.user.email"></v-divider>
          </template>
        </v-list>
